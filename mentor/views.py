...
@login_required
def mentor_material_upload_file_view(request):
...
@login_required
def mentor_material_start_upload_file_view(request):
    if request.method=="POST":
...
@login_required
def mentor_upload_material_folder_view(request):
...
def mentor_start_upload_material_folder_view(request):
    if request.method=="POST":
...
@login_required
def mentor_activity_upload_file_view(request):
    subjects = LXPModel.Playlist.objects.all()
    context = {'subjects': subjects}
    return render(request,'mentor/uploadpdf/mentor_activity_upload_file.html',context)
...
@login_required
def mentor_activity_start_upload_file_view(request):
    if request.method=="POST":
        file=request.FILES["select_file"]
        if file == "":
            return "Please return to previous page and select a file"
...
@login_required
def mentor_upload_activity_folder_view(request):
    return render(request,'mentor/activity/mentor_Upload_activity_folder.html')
...
def mentor_start_upload_activity_folder_view(request):
    if request.method=="POST":
        folder=request.POST["select_folder"]
        folder = str.replace(folder,'/','\\')
        path ='D:\\upload\\iLMS'
