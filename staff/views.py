...
@login_required
def staff_material_upload_file_view(request):
...
@login_required
def staff_material_start_upload_file_view(request):
    if request.method=="POST":
...
@login_required
def staff_upload_material_folder_view(request):
...
def staff_start_upload_material_folder_view(request):
    if request.method=="POST":
...
@login_required
def staff_scheduler_calender(request):
    schedulers = LXPModel.Scheduler.objects.annotate(
        status_sum=Coalesce(Sum('schedulerstatus__status'), Value(0)),
        completion_date=Case(
            When(status_sum__gte=100, then=Max('schedulerstatus__date')),
    ...
    return render(request, 'staff/calender/staff_calender.html', {'schedulers': schedulers})


@login_required
def staff_calender(request):
    # Get schedulers for the logged-in staff and use Coalesce to replace None with 0 for status_sum
    schedulers = LXPModel.Scheduler.objects.filter(
        staff_id=request.user.id
    ).annotate(
        status_sum=Coalesce(Sum('schedulerstatus__status'), Value(0))
    )
    return render(request, 'staff/scheduler/staff_calender.html', {'schedulers': schedulers})

# Display list of schedulerstatus
@login_required
def staff_schedulerstatus_list(request):
    if str(request.session['utype']) == 'staff':
        schedulerstatus = LXPModel.SchedulerStatus.objects.all()
        return render(request, 'staff/schedulerstatus/staff_schedulerstatus_list.html', {'schedulerstatus': schedulerstatus})
    else:
        return render(request,'loginrelated/diffrentuser.html')
        ...
        if request.method == 'POST':
            ...
            if status_sum:
                if (float(status) < float(status_sum)) :
                    ...
                    return redirect('staff-schedulerstatus-create')
            ...
            return redirect('staff-schedulerstatus-create')
        schedulers = LXPModel.Scheduler.objects.filter(staff_id = request.user.id)
        
        return render(request, 'staff/schedulerstatus/staff_schedulerstatus_create.html',{'schedulers':schedulers})
    else:
        return render(request,'loginrelated/diffrentuser.html')
...
def schedulerstatus_delete(request, id):
    if str(request.session['utype']) == 'staff':
        ...
        return redirect('staff-schedulerstatus-list')
    else:
        return render(request,'loginrelated/diffrentuser.html')
...
@login_required
@csrf_exempt
def staff_schedulerstatus_mark_done(request):
    if request.method == 'POST':
        try:
